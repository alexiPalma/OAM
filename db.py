...
async def init_db():
    db=await connect();cols=', '.join(f'{k} INTEGER NOT NULL DEFAULT 0' for k in UNITS);kills=', '.join(f'kill_{k} INTEGER NOT NULL DEFAULT 0' for k in UNITS)
    await db.execute(f'''CREATE TABLE IF NOT EXISTS users(user_id INTEGER PRIMARY KEY,username TEXT NOT NULL DEFAULT '',balance INTEGER NOT NULL DEFAULT 0,farm_level INTEGER NOT NULL DEFAULT 0,tax INTEGER NOT NULL DEFAULT 0,last_payout TEXT NOT NULL,daily_claim TEXT NOT NULL DEFAULT '',sub_claim INTEGER NOT NULL DEFAULT 0,last_attack TEXT NOT NULL DEFAULT '',attacks_won INTEGER NOT NULL DEFAULT 0,attacks_lost INTEGER NOT NULL DEFAULT 0,{cols},{kills})''')
...
async def ensure_user(uid,username=''):
    db=await connect();stamp=datetime.now(timezone.utc).isoformat();await db.execute('INSERT OR IGNORE INTO users(user_id,username,last_payout,farm_level) VALUES(?,?,?,0)',(uid,username or '',stamp));await db.execute('UPDATE users SET username=? WHERE user_id=?',(username or '',uid));await db.commit();await db.close()
async def user(uid):
