...
# Achievements are based on DESTROYED units, not owned units.
# Thresholds are kept from the existing achievement progression.
ACHIEVEMENTS = [
...
def reward_text(rewards):
    ...
    for kind,amount in rewards:
        amount=int(amount)
        if kind=='money': out.append(f'💵 ${amount:,}'.replace(',',' '))
        else: out.append(f'{REWARD_NAMES.get(kind,kind)} × {amount:,}'.replace(',',' '))
    return '\n'.join(out)
...
async def menu(c):
    ...
    await app.safe(c,f'🏆 {app.BRAND} • АЧИВКИ\n\nАчивки выдаются за количество уничтоженной техники.\nНажмите на ачивку для подробностей.',app.kb(rows))

async def detail(c,aid):
    item=next((x for x in ACHIEVEMENTS if x[0]==aid),None)
    if item is None:return await c.answer('Ачивка не найдена.',show_alert=True)
    _,title,requirements,rewards=item;await check(c.from_user.id,c.bot,False);row=await user(c.from_user.id)
    db=await connect()
    try:
    ...
    req='\n'.join(f'{REQ_NAMES[i]}: {int(row[k] or 0)}/{need}' for i,(k,need) in enumerate(zip(REQ_KEYS,requirements)) if need) or 'Нет требований.'
    done=bool(state and int(state['completed']));claimed=bool(state and int(state['claimed']));buttons=[]
...
async def claim(c,aid):
    item=next((x for x in ACHIEVEMENTS if x[0]==aid),None)
    if item is None:return await c.answer('Ачивка не найдена.',show_alert=True)
    _,title,_requirements,rewards=item;await check(c.from_user.id,c.bot,False)
    db=await connect()
    try:
        cur=await db.execute('SELECT completed,claimed FROM achievements WHERE user_id=? AND achievement_id=?',(c.from_user.id,aid));state=await cur.fetchone()
        ...
        if sets:
            params.append(c.from_user.id);await db.execute(f'UPDATE users SET {",".join(sets)} WHERE user_id=?',params)
...
def install_sync():
    # run.py owns the achievement callback bridge. Do not overwrite bot callbacks.
    return None
