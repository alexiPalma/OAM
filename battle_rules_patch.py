...
def _money(value): return f'{int(value):,}'.replace(',', ' ')
...
async def _edit(tg_bot, chat_id, message_id, text, markup=None):
    ...
    try:
        await tg_bot.edit_message_text(chat_id=chat_id, message_id=message_id, text=text, reply_markup=markup, parse_mode=None)
    except Exception:
        pass
...
async def battle_accept(c, attacker_id, tg_bot):
    ...
    if not attacker or not defender:
        app.INVITES.pop(attacker_id, None); _PENDING_MESSAGES.pop(attacker_id, None)
    ...
    try:
        ...
        await db.execute(f'UPDATE users SET {unit_cols},attacks_won=attacks_won+1,last_attack=? WHERE user_id=?',[winner_arm[k] for k in UNITS] + [_now(), winner_id])
        await db.execute(f'UPDATE users SET {unit_cols},attacks_lost=attacks_lost+1,last_attack=? WHERE user_id=?',[loser_arm[k] for k in UNITS] + [_now(), loser_id])
        await db.execute(f'UPDATE users SET {kill_cols} WHERE user_id=?',[winner_kills[k] for k in UNITS] + [winner_id])
        await db.execute(f'UPDATE users SET {kill_cols} WHERE user_id=?',[loser_kills[k] for k in UNITS] + [loser_id])
    ...
    try: win_text = await app.tpl('win', win_default, winner=winner_name, reward=_money(reward), kills=winner_kill_text)
    except Exception: win_text = win_default
    try: loss_text = await app.tpl('loss', loss_default, winner=winner_name, loss='20%', reward=_money(loser_reward), kills=loser_kill_text)
    except Exception: loss_text = loss_default

    await asyncio.gather(
        _edit(tg_bot, attacker_id, attacker_message_id, win_text if winner_id == attacker_id else loss_text, app.back()),
...
# Do not depend solely on a particular launcher importing this module. The
# repository already uses sitecustomize as a bootstrap, so importing this
# module now activates the patch exactly once even when another launcher is
# used.
install(app)
