...
# The project has several historical patch layers. Install one final bridge at
# import time so the real run.py gets the fixes without replacing its feature set.
...
def _patch_run(run):
    ...
    import random
    from db import connect, top_users, user
    import bot as app

    # Weighted top: 1/1/3/7/8/10/15/25/50, with the rating shown explicitly.
    ...
    # Artillery must be visible and purchasable in the arsenal.
    ...
    async def fixed_add_promo(message,parts):
        ...
        if len(parts)==3: code,amount_s,max_s=parts
            ...
            if unit not in UNITS:return await message.answer('❌ Неизвестная техника: '+', '.join(UNITS))
        ...
        except ValueError:return await message.answer('❌ Сумма/количество и лимит должны быть числами.')
