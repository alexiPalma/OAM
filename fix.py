...
# This state is owned here. We do not depend on a second battle module.
INVITES = {}
PENDING = {}
...
def kills_text(kills):
    return '\n'.join(
        f'{UNITS[k]["title"]}: {int(kills.get(k, 0))}'
        for k in UNITS
    )
...
def invite_is_valid(attacker_id, defender_id):
    ...
    if not item:
        return False
    if int(item['defender_id']) != int(defender_id):
        return False
...
async def battle_confirm(c, bot):
    ...
    # Do not create an invitation and then hand it to another callback module.
    # The invitation is created here atomically before Telegram sends the request.
    app.PENDING.pop(attacker_id, None)
...
async def battle_decline(c, attacker_id, bot):
    ...
    INVITES.pop(attacker_id, None)
    # Keep the existing decline mechanics from bot.py, but with our own state.
...
async def battle_accept(c, attacker_id, bot):
    ...
    # Validate against our own invitation, not app.INVITES. This removes the
    # exact failure where Telegram showed a fresh button but another runtime
    # module had already lost the invitation from its dictionary.
    if not invite_is_valid(attacker_id, defender_id):
        return await c.answer('Предложение уже недействительно.', show_alert=True)
    ...
    # Both users receive the same frame from the same coroutine and the same
    # frame index. There is no separate 15-second loop for the attacker.
    start_text = '⚔️ БОЙ\n\nБой начинается...\n\n⏱ 15 сек.'
    ...
    # Result is delivered to both only after the final animation frame.
    await asyncio.gather(
        c.message.edit_text(
