...
# ---------- config: equipment order, artillery price, rating ----------
try:
    import config
    order = [
...
# ---------- import the existing bot after config changes ----------
try:
    import bot as app
    from db import connect, user, top_users
    from config import FARMS, UNITS, DAILY_BONUS_PRIZES
    from aiogram.types import InlineKeyboardButton, InlineKeyboardMarkup

    # Preserve the URL-button fix: URL values are real Telegram URL buttons.
    def fixed_kb(rows):
        keyboard = []
        for row in rows:
            buttons = []
    ...
    async def fixed_top_users(limit=50):
        ...
        cur = await db.execute(
            f'SELECT *, ({expr}) AS army_total FROM users '
    ...
    async def fixed_farm(c):
        ...
        text = await app.tpl(
            'farm',
            f'🏭 {app.BRAND} • ФЕРМА\n\nУровень: {level}/10\n'
            f'Производство: ${app.money(farm_data["income"])}/час\n'
            f'💸 Накоплено налога: ${tax}\nСтавка налога: 25%\nСтатус: {status}',
            level=level, income=app.money(farm_data['income']), tax=tax, status=status,
        )
        return await app.safe(c, text, app.kb([
            [('💰 Получить', 'payout'), ('⬆️ Улучшить', 'upgrade')],
            [('💸 Оплатить налог', 'paytax')], [('⬅️ Назад', 'home')],
        ]))
    app.farm = fixed_farm
        ...
        async def text_handler_keywords(m, bot, *args, **kwargs):
            ...
            if low in ('help', 'хелп', '/help', '/хелп'):
                return await m.answer(
                    f'ℹ️ {app.BRAND} • ПОМОЩЬ\n\nРазвивайте ферму, покупайте армию и участвуйте в боях.',
    ...
    # ---------- achievements: destroy counts, not owned counts ----------
    try:
        import achievements
        achievements.REQ_KEYS = tuple(f'kill_{k}' for k in (
            'soldier', 'drone', 'tank', 'bmp', 'helicopter', 'plane', 'missile', 'interceptor'
        ))
        achievements.REQ_NAMES = (
            '💀 Уничтожено солдат', '💀 Уничтожено БПЛА', '💀 Уничтожено танков',
            '💀 Уничтожено БМП', '💀 Уничтожено вертолётов', '💀 Уничтожено самолётов',
    ...
    try:
        ...
        if not getattr(combat, '_wwd_artillery_patch', False):
            ...
            def resolve_with_artillery(attacker, defender, with_kills=False):
                ...
                ka = {k: 0 for k in UNITS}
                kd = {k: 0 for k in UNITS}
                ...
                for key in UNITS:
                    ka[key] += old_ka.get(key, 0)
                    kd[key] += old_kd.get(key, 0)
                aa['artillery'] = max(0, int(a['artillery']) - kd.get('artillery', 0))
                dd['artillery'] = max(0, int(d['artillery']) - ka.get('artillery', 0))
                if with_kills:
                    return aa, dd, winner, events, ka, kd
                return aa, dd, winner, events
            combat.resolve = resolve_with_artillery
            combat._wwd_artillery_patch = True
    except Exception:
        pass

    # ---------- trace run.py to patch functions defined in __main__ ----------
    def patch_run_main(frame):
        ...
        if home is not None and not getattr(home, '_wwd_ach_patch', False):
            def home_with_achievements(is_admin_user=False):
                markup = home(is_admin_user)
                rows = [list(row) for row in markup.inline_keyboard]
                if not any(b.callback_data == 'achievements' for row in rows for b in row):
                    rows.insert(max(0, len(rows) - (1 if is_admin_user else 0)), [InlineKeyboardButton(text='🏆 Ачивки', callback_data='achievements')])
                return InlineKeyboardMarkup(inline_keyboard=rows)
            home_with_achievements._wwd_ach_patch = True
            g['home_kb'] = home_with_achievements

        if all(name in g for name in ('add_promo', 'use_promo_extended', 'cases', 'case')) and not g.get('_wwd_cases_patched'):
            g['_wwd_cases_patched'] = True
            old_add = g['add_promo']
            old_use = g['use_promo_extended']
            old_cases = g['cases']
            old_case = g['case']

            async def ensure_case_table():
                db = await connect()
                await db.execute('''CREATE TABLE IF NOT EXISTS promo_case_inventory(
            ...
            async def add_promo(message, parts):
                if len(parts) == 5 and parts[1].lower() in ('case', 'кейс'):
                    ...
                    await ensure_case_table()
                    db = await connect()
                    await db.execute('''INSERT INTO promos(code,amount,uses,max_uses,reward_type,reward_amount)
                        VALUES(?,0,0,?,?,?)
                        ON CONFLICT(code) DO UPDATE SET amount=0,max_uses=excluded.max_uses,
                        reward_type=excluded.reward_type,reward_amount=excluded.reward_amount''',
                        (code, int(max_s), 'case:' + case_id, int(amount_s)))
                    await db.commit(); await db.close()
            ...
            async def use_promo_extended(message, code):
                ...
                if await cur.fetchone():
                    await db.close(); return await message.answer('❌ Вы уже использовали этот промокод.')
                await db.execute('CREATE TABLE IF NOT EXISTS promo_case_inventory(user_id INTEGER PRIMARY KEY,case1 INTEGER NOT NULL DEFAULT 0,case2 INTEGER NOT NULL DEFAULT 0,president INTEGER NOT NULL DEFAULT 0)')
                case_id = str(promo['reward_type']).split(':', 1)[1]
                amount = int(promo['reward_amount'] or 1)
                await db.execute('INSERT OR IGNORE INTO promo_case_inventory(user_id) VALUES(?)', (message.from_user.id,))
                await db.execute(f'UPDATE promo_case_inventory SET {case_id}={case_id}+? WHERE user_id=?', (amount, message.from_user.id))
            ...
            async def cases(c):
                await ensure_case_table()
                db = await connect(); cur = await db.execute('SELECT case1,case2,president FROM promo_case_inventory WHERE user_id=?', (c.from_user.id,)); row = await cur.fetchone(); await db.close()
                if not row:
                    row = {'case1':0,'case2':0,'president':0}
                text = (f'📦 {g["app"].BRAND} • КЕЙСЫ\n\n'
                        f'📦 Кейс 1 — $45 000\nБесплатных: {int(row["case1"])}\n\n'
                        f'📦 Кейс 2 — $5 000 000\nБесплатных: {int(row["case2"])}\n\n'
                        f'🎖 Президентский — 50 ⭐\nБесплатных: {int(row["president"])}')
                return await g['app'].safe(c, text, g['app'].kb([[('📦 Кейс 1','case1'),('📦 Кейс 2','case2')],[('🎖 Президентский','president')],[('⬅️ Назад','home')]]))
            ...
            async def case(c, cid):
                ...
                await ensure_case_table()
                db = await connect()
                cur = await db.execute(f'UPDATE promo_case_inventory SET {cid}={cid}-1 WHERE user_id=? AND {cid}>0', (c.from_user.id,))
                await db.commit(); await db.close()
                if cur.rowcount != 1:
                    return await old_case(c, cid)
                pools = {
                    'case1': [('soldier',2,75),('interceptor',11,15),('soldier',10,10)],
                ...
                for unit, amount, chance in pools[cid]:
                    acc += chance
                    if r < acc: break
                db = await connect(); await db.execute(f'UPDATE users SET {unit}={unit}+? WHERE user_id=?', (amount, c.from_user.id)); await db.commit(); await db.close()
                return await g['app'].safe(c, f'📦 Бесплатный кейс открыт!\n\n{UNITS[unit]["title"]} × {amount}\n🎲 Шанс: {chance}%', g['app'].back('cases'))

            g['add_promo'] = add_promo
            g['use_promo_extended'] = use_promo_extended
            g['cases'] = cases
            g['case'] = case
        return True
    ...
    def trace(frame, event, arg):
        if event in ('line', 'return'):
            try:
                if patch_run_main(frame):
                    if frame.f_globals.get('_wwd_cases_patched'):
                        sys.settrace(None)
                        return None
            except Exception:
                pass
