...
BRAND='WorldWarDynasty'; STATE={}; BATTLES={}; NEXT_BATTLE=1
...
def home_kb(admin=False):
    r=[[('🏭 Ферма','farm'),('🎖 Армия','army')],[('🛒 Арсенал','shop'),('⚔️ Атака','attack')],[('🎁 Бонус','bonus'),('📦 Кейсы','cases')],[('👤 Профиль','profile'),('🏆 Топ вояк','top')],[('💳 Донат','donate'),('📕 Правила','rules')],[('ℹ️ Помощь','help')]]
...
async def msg(key,default): return await setting('msg_'+key,await setting(key,default))
...
async def daily(c):
    ...
    prize=random.choice([(100000,'money'),(10,'interceptor'),(2,'drone'),(1,'bmp'),(10,'drone'),(1,'tank'),(300000,'money'),(50,'interceptor'),(1,'helicopter')]); amount,unit=prize; db=await connect(); col='balance' if unit=='money' else unit; await db.execute(f'UPDATE users SET {col}={col}+?,daily_claim=? WHERE user_id=?',(amount,today,c.from_user.id)); await db.commit(); await db.close(); await safe(c,f'🎁 Вы получили <b>{"$"+money(amount) if unit=="money" else UNITS[unit]["title"]+" × "+str(amount)}</b>.',back('bonus'))
async def sub(c,bot):
...
async def earn_admin(c):
    ...
    await safe(c,f'📢 <b>{BRAND} • ЗАРАБОТАТЬ</b>\n\nВыберите тип задания:',earn_admin_keyboard())
async def earn_list(c):
    db=await connect(); cur=await db.execute('SELECT id,kind,url,reward FROM earn_tasks WHERE active=1 ORDER BY id'); items=await cur.fetchall(); await db.close();
    data=[]
    for x in items:data.append({'id':x['id'],'kind':x['kind'],'url':x['url'],'reward':x['reward'],'title':{'boost':'🚀 Буст канала','channel':'📢 Подписка на канал','group':'👥 Подписка на группу'}[x['kind']]})
    await safe(c,'📢 <b>Доступные задания</b>',earn_player_keyboard(data))
async def earn_add(c,kind):
    if not await admin(c.from_user.id):return await c.answer('Нет доступа',show_alert=True)
    STATE[c.from_user.id]=('earn_add',kind); await safe(c,f'Введите команду вида <code>/add{kind} https://t.me/... 150000</code>',back('a_earn'))
async def earn_check(c,kind,tid,bot):
    db=await connect(); cur=await db.execute('SELECT * FROM earn_tasks WHERE id=? AND kind=? AND active=1',(tid,kind)); task=await cur.fetchone(); await db.close()
    if not task:return await c.answer('Задание не найдено.',show_alert=True)
    db=await connect(); cur=await db.execute('SELECT 1 FROM earn_claims WHERE user_id=? AND task_id=?',(c.from_user.id,tid)); claimed=await cur.fetchone(); await db.close()
    if claimed:return await c.answer('Награда уже получена.',show_alert=True)
    m=re.match(r'https?://t\.me/([A-Za-z0-9_]+)$',task['url'].rstrip('/'))
    if not m:return await c.answer('Для проверки нужна публичная ссылка.',show_alert=True)
    try:
        member=await bot.get_chat_member('@'+m.group(1),c.from_user.id); ok=member.status not in ('left','kicked')
...
async def admin_panel(c):
    if not await admin(c.from_user.id):return await c.answer('Нет доступа',show_alert=True)
    await safe(c,f'⚙️ <b>{BRAND} • АДМИН-ПАНЕЛЬ</b>\n\nУправление экономикой, армиями, кейсами, заданиями и сообщениями.',admin_kb())
async def admin_section(c,s):
    ...
    if s=='a_donate':return await safe(c,'💳 <b>ДОНАТ</b>\n\n50 ⭐ — $5 000 000\n100 ⭐ — $11 000 000\n500 ⭐ — $100 000 000',back('admin'))
    if s=='a_cases':return await cases(c)
    if s=='a_edit':return await safe(c,'✏️ <b>РЕДАКТИРОВАНИЕ</b>\n\nВ этой версии системные сообщения хранятся в settings и могут быть расширены без изменения игрового кода.',back('admin'))
    if s=='a_give':return await safe(c,'🎖 <b>ВЫДАЧА</b>\n\n/givepehot @username ID количество\n/givecash @username сумма\n\nID: 1 пехота · 2 перехватчик · 3 БПЛА · 4 БМП · 5 танк · 6 вертолёт · 7 самолёт · 8 ракета · 9 артиллерия',back('admin'))
    if s=='a_admins':return await safe(c,'👥 <b>АДМИНЫ</b>\n\nДобавление/удаление выполняется владельцем через команды администратора.',back('admin'))
    await safe(c,f'⚙️ <b>{BRAND}</b>\n\nРаздел <b>{esc(s)}</b> готов к настройке.',back('admin'))
...
async def text_handler(m,bot):
    text=(m.text or '').strip(); st=STATE.get(m.from_user.id)
    if text.startswith('/'):
        ...
        if cmd in ('/addboost','/addchannel','/addgroup'):
            ...
            url,reward=parsed; db=await connect(); await db.execute('INSERT OR IGNORE INTO earn_tasks(kind,url,reward) VALUES(?,?,?)',(kind,url,reward)); await db.commit(); await db.close(); return await m.answer('✅ Задание добавлено.')
        if cmd=='/givecash' and await admin(m.from_user.id) and len(p)==3:
            ...
            db=await connect();await db.execute('UPDATE users SET balance=balance+? WHERE user_id=?',(amount,target['user_id']));await db.commit();await db.close();return await m.answer('✅ Выдано.')
    ...
    if typ=='buy':
        ...
        STATE.pop(m.from_user.id,None);price=UNITS[val]['price']*q
        if q<=0:return await m.answer('❌ Некорректное количество.')
        await m.answer(f'🛒 {UNITS[val]["title"]} × {q}\n💵 ${money(price)}',reply_markup=kb([[('✅ Купить',f'buyok:{val}:{q}')],[('❌ Отмена','shop')]]),parse_mode='HTML');return
    if typ=='earn_add':
        cmd={'boost':'/addboost','channel':'/addchannel','group':'/addgroup'}[val]; STATE.pop(m.from_user.id,None); fake=text if text.startswith('/') else cmd+' '+text; parsed,err=parse_earn_command(fake,cmd)
        if err:return await m.answer('❌ '+err)
        url,reward=parsed;db=await connect();await db.execute('INSERT OR IGNORE INTO earn_tasks(kind,url,reward) VALUES(?,?,?)',(val,url,reward));await db.commit();await db.close();return await m.answer('✅ Задание добавлено.')

async def callback(c,bot):
    d=c.data or ''
    if d=='home':return await safe(c,(f'⚔️ <b>{BRAND}</b>\n\n💵 Баланс: <b>${money((await user(c.from_user.id))["balance"])}</b>'),home_kb(await admin(c.from_user.id)))
    ...
    if d.startswith('case:') or d in ('case1','case2','president'):return await case(c,d.split(':',1)[1] if ':' in d else d)
    if d=='donate':return await donate(c)
    if d=='top':return await top(c)
    if d=='help':return await safe(c,f'ℹ️ <b>{BRAND} • ПОМОЩЬ</b>\n\nИспользуйте /start. Покупайте войска, развивайте ферму и участвуйте в боях.',back())
    if d=='rules':return await safe(c,f'📕 <b>{BRAND} • ПРАВИЛА</b>\n\n1. Одна атака в час.\n2. После подтверждения бой идёт 15 секунд.\n3. Проигравший теряет 20% армии.\n4. Победитель получает 5% стоимости уничтоженного.\n5. Проигравший получает 2% стоимости потерь.',back())
    if d=='attack':return await safe(c,'⚔️ <b>АТАКА</b>\n\nСистема боя подключена. Выберите противника из игрового интерфейса.',back())
    if d=='admin':return await admin_panel(c)
    if d.startswith('a_'):return await admin_section(c,d)
    if d=='earn_list':return await earn_list(c)
