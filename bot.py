...
async def battle_accept(c,attacker_id,bot):
    ...
    unit_titles={'soldier':'🪖 Пехота','interceptor':'🎯 Перехватчики','drone':'🛩 БПЛА','bmp':'🚙 БМП','tank':'🛡 Танки','helicopter':'🚁 Вертолёты','plane':'✈️ Самолёты','missile':'🚀 Ракеты','artillery':'💥 Артиллерия'}
    kills_w='\n'.join(f'{unit_titles[k]}: {int(winner_k.get(k,0))}' for k in unit_titles)
    kills_l='\n'.join(f'{unit_titles[k]}: {int(loser_k.get(k,0))}' for k in unit_titles)
    ...
    await bot.send_message(winner_id,wintext,reply_markup=back())
    await bot.send_message(loser_id,losstext,reply_markup=back())
