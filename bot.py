...
BRAND='WorldWarDynasty'; STATE={}; ATTACK_CD=timedelta(minutes=10); PENDING={}
...
async def farm(c):
    u=await user(c.from_user.id);f=FARMS[int(u['farm_level'])];status='🟢 АКТИВНА' if int(u['farm_level'])>0 else '⚪ НЕ РАЗВЁРНУТА'
...
async def payout(c):
    ...
    income=int(FARMS[lvl]['income']);tax=int(income*0.25);db=await connect();await db.execute('UPDATE users SET balance=balance+?,last_payout=?,tax=? WHERE user_id=?',(income,now().isoformat(),tax,c.from_user.id));await db.commit();await db.close();await safe(c,f'💰 Получено +${money(income)}\n💸 Налог 25%: ${money(tax)}.',back('farm'))
async def paytax(c):
...
async def opponent(c,uid):
    ...
    PENDING[c.from_user.id]=uid;name='@'+opp['username'] if opp['username'] else f'ID {uid}';await safe(c,f'🎯 {BRAND} • ПРОТИВНИК\n\n👤 {name}\n\n{army_text(opp)}',kb([[('⚔️ НАПАСТЬ','battle_confirm')],[('⬅️ Назад','attack')]]))
async def battle_confirm(c,bot):
    ...
    a_after,d_after,winner,events,kills_a,kills_d=resolve(me,opp,with_kills=True);winner_id=c.from_user.id if winner=='attacker' else uid;loser_id=uid if winner=='attacker' else c.from_user.id;winner_arm=a_after if winner=='attacker' else d_after;loser_arm=d_after if winner=='attacker' else a_after;loser_arm={k:int(loser_arm[k])*80//100 for k in UNITS};winner_k=kills_a if winner=='attacker' else kills_d;loser_k=kills_d if winner=='attacker' else kills_a;reward=int(sum(winner_k[k]*UNITS[k]['price'] for k in UNITS)*0.05);loser_reward=int(sum((int((opp if winner=='attacker' else me)[k])-loser_arm[k])*UNITS[k]['price'] for k in UNITS)*0.02)
    db=await connect();sets=','.join(f'{k}=?' for k in UNITS);ksets=','.join(f'kill_{k}=kill_{k}+?' for k in UNITS);await db.execute(f'UPDATE users SET {sets},attacks_won=attacks_won+1,last_attack=? WHERE user_id=?',[winner_arm[k] for k in UNITS]+[now().isoformat(),winner_id]);await db.execute(f'UPDATE users SET {sets},attacks_lost=attacks_lost+1,last_attack=? WHERE user_id=?',[loser_arm[k] for k in UNITS]+[now().isoformat(),loser_id]);await db.execute(f'UPDATE users SET {ksets} WHERE user_id=?',[winner_k[k] for k in UNITS]+[winner_id]);await db.execute(f'UPDATE users SET {ksets} WHERE user_id=?',[loser_k[k] for k in UNITS]+[loser_id]);await db.execute('UPDATE users SET balance=balance+? WHERE user_id=?',(reward,winner_id));await db.execute('UPDATE users SET balance=balance+? WHERE user_id=?',(loser_reward,loser_id));await db.execute('INSERT INTO battle_log(attacker,defender,winner,report,created_at) VALUES(?,?,?,?,?)',(c.from_user.id,uid,winner_id,'\n'.join(events[-25:]),now().isoformat()));await db.commit();await db.close();report='\n'.join(events[-25:]) or 'Бой завершён.'
    result=f'🏆 WIN\n\n💰 Победитель получил: ${money(reward)}\n📉 Армия проигравшего: −20%\n💵 Проигравшему: +${money(loser_reward)}\n\n{report}' if winner=='attacker' else f'💀 LOSS\n\n📉 Твоя армия: −20%\n💵 Компенсация: +${money(loser_reward)}\n\n{report}';await safe(c,result,back())
...
async def admin_section(c,s):
    ...
    if s=='a_bonus':return await safe(c,f'🎁 БОНУСЫ\n\nЕжедневный бонус использует заданные шансы 50 / 20 / 10 / 5 / 5 / 4.9 / 2.5 / 2.5 / 0.1%.',back('admin'))
...
async def main():
    ...
    db=await connect();await db.execute('INSERT OR IGNORE INTO admins(user_id) VALUES(?)',(ADMIN_ID,));
    if OWNER_ID2:await db.execute('INSERT OR IGNORE INTO admins(user_id) VALUES(?)',(OWNER_ID2,))
    await db.commit();await db.close()
    tg=Bot(BOT_TOKEN);dp=Dispatcher();dp.message.register(start,CommandStart());dp.message.register(text_handler,F.text);dp.callback_query.register(callback,F.data);print(f'{BRAND} started');await dp.start_polling(tg)
