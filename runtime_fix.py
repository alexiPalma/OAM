...
async def farm(c):
    ...
    await bot.safe(c,f'🏭 {bot.BRAND} • ФЕРМА\n\nУровень: {lvl}/10\nПроизводство: ${_money(f["income"])}/час\nНалог к оплате: ${_money(tax)}\nСтавка налога: 25%\nСтатус: {status}',bot.kb([[('💰 Получить','payout'),('⬆️ Улучшить','upgrade')],[('💸 Оплатить налог','paytax')],[('⬅️ Назад','home')]]))

async def payout(c):
    u=await bot.user(c.from_user.id); lvl=int(u['farm_level'])
    if lvl<=0:return await bot.safe(c,'🏭 Сначала улучшите ферму до 1 уровня за $500 000.',bot.back('farm'))
    if int(u['tax'])>0:return await bot.safe(c,f'❌ Сначала оплатите налог: ${_money(u["tax"])}.',bot.back('farm'))
    try:
        last=datetime.fromisoformat(u['last_payout'])
        if last.tzinfo is None:last=last.replace(tzinfo=timezone.utc)
    except Exception:last=_now()-timedelta(hours=1)
    if _now()-last<timedelta(hours=1):return await bot.safe(c,'⏳ Выплата доступна один раз в час.',bot.back('farm'))
...
async def paytax(c):
    ...
    if tax<=0:return await bot.safe(c,'✅ Налог к оплате отсутствует.',bot.back('farm'))
    if int(u['balance'])<tax:return await bot.safe(c,f'❌ Недостаточно средств. Нужно ${_money(tax)}.',bot.back('farm'))
    db=await connect();await db.execute('UPDATE users SET balance=balance-?,tax=0 WHERE user_id=?',(tax,c.from_user.id));await db.commit();await db.close();await bot.safe(c,f'✅ Налог ${_money(tax)} оплачен.',bot.back('farm'))
...
# IMPORTANT: aiogram injects the Telegram Bot instance under the parameter name 'bot'.
# Using 'tg_bot' makes dependency injection fail with: callback() missing ... 'tg_bot'.
async def text_handler(m,bot_instance):
    ...
    if cmd=='/promo' and len(p)==2:
        await bot.ensure_user(m.from_user.id,m.from_user.username);db=await connect();cur=await db.execute('SELECT code,amount,uses,max_uses FROM promos WHERE lower(code)=lower(?)',(p[1],));pr=await cur.fetchone()
    ...
    if await _admin(m.from_user.id):
        ...
        if cmd=='/broadcast' and len(p)>1:
            ...
            for uid in ids:
                try:await bot_instance.send_message(uid,text.split(maxsplit=1)[1]);sent+=1
                except Exception:pass
            return await m.answer(f'📣 Рассылка завершена: {sent}/{len(ids)}')
    return await bot._original_text_handler(m,bot_instance)

# IMPORTANT: parameter name must be 'bot' because aiogram injects Bot by name.
async def callback(c,bot_instance):
    d=c.data or ''
    try:
        ...
        return await bot._original_callback(c,bot_instance)
...
def install():
    ...
    bot.admin_section=admin_section;bot.text_handler=text_handler;bot.callback=callback
    bot.farm=farm;bot.payout=payout;bot.paytax=paytax;bot.upgrade=upgrade
