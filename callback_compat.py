...
async def _takeunit(m, parts):
    ...
    return await m.answer(f"✅ Списано: {amount} × {UNITS[unit]['title']} у @{target['username'] or target['user_id']}.") if cur.rowcount==1 else await m.answer('❌ Не удалось изменить армию пользователя.')
...
def install():
    ...
    async def text_handler(m,bot):
        text=(m.text or '').strip();parts=text.split()
