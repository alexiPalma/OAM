...
This launcher deliberately avoids the nested runtime patchers.  It imports the
real application and registers its own dispatcher, so callback handlers receive
aiogram's Bot dependency correctly.
"""
import asyncio
import contextvars
...
# All keyboards generated by the real bot now use the same group ownership rule.
app.kb = scoped_kb
...
async def addpromo(m,p):
    if not await admin_access(m.from_user.id):return await m.answer("⛔ Нет доступа.")
    if len(p)==3:code,amount_s,limit_s=p;reward_type='money'
...
async def callback(c,tg_bot:Bot):
    data=c.data or '';group=bool(c.message and c.message.chat.type!='private')
...
async def text_handler(m,tg_bot:Bot):
    ...
    if low in ('промо','промокод','promo','/promo','/промо','/промокод'):PROMO_WAIT.add(m.from_user.id);return await m.answer('🎟 Введите промокод:')
